...
for count in range(2,policies+1):
	...
	relations = {'>': Lt,'>=': Le,'<': Lt,'<=': Le,'==': CustomEq,'!=': Ne}
	inequalities = []
	...
	inequalities = [str(ineq) for ineq in inequalities]
	inequalities = ", ".join(inequalities)
	# print(dir(Lt))
	# print(Lt(s0, s1))
	# print(CustomEq(s0, s1))
	# break

	comb += "\n\nValid constraints of " + str(count) + " inputs:\n" + order + " such that "+ str(inequalities)
...
with open(name+"_normal.txt", "w") as f:
	f.write(comb)

# with open(name+"_chain.txt", "w") as f:
# 	f.write(comb)	